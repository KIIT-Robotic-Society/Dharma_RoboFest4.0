...
void getSequenceStepEnables(SequenceStepEnables * enables, statInfo_t_VL53L0X *sensorInfo);
void getSequenceStepEnables_x(SequenceStepEnables * enables);
void getSequenceStepTimeouts(SequenceStepEnables const * enables, SequenceStepTimeouts * timeouts,statInfo_t_VL53L0X *sensorInfo);
...
// Write a 32-bit register
void writeReg32Bit_x(uint8_t reg, uint32_t value, statInfo_t_VL53L0X *sensorInfo){

  memcpy(msgBuffer, &value, 4); // Assign the value to the buffer.
  i2cStat = HAL_I2C_Mem_Write(&VL53L0X_I2C_Handler, sensorInfo->address | I2C_WRITE, reg, 1, msgBuffer, 4, I2C_TIMEOUT);
}
...
// Read a 32-bit register
uint32_t readReg32Bit_x(uint8_t reg, statInfo_t_VL53L0X *sensorInfo) {
  uint32_t value;
  i2cStat = HAL_I2C_Mem_Read(&VL53L0X_I2C_Handler, sensorInfo->address  | I2C_READ, reg, 1, msgBuffer, 4, I2C_TIMEOUT);
  memcpy(&value, msgBuffer, 4);
...
void writeMulti_x(uint8_t reg, uint8_t const *src, uint8_t count, statInfo_t_VL53L0X *sensorInfo){
...
/*bool initVL53L0X(bool io_2v8, I2C_HandleTypeDef *handler){
  ...
  setMeasurementTimingBudget_x(g_measTimBudUs);
...
/*bool setSignalRateLimit(float limit_Mcps, statInfo_t_VL53L0X *sensorInfo)
{
...
bool setSignalRateLimit_x(float limit_Mcps)
{
  if (limit_Mcps < 0 || limit_Mcps > 511.99) { return false; }
...
/*bool setMeasurementTimingBudget(uint32_t budget_us, statInfo_t_VL53L0X *sensorInfo)
{
  ...
  getSequenceStepEnables(&enables,  sensorInfo->address);
  getSequenceStepTimeouts(&enables, &timeouts,  sensorInfo->address);
...
bool setMeasurementTimingBudget_x(uint32_t budget_us)
{
  SequenceStepEnables enables;
  SequenceStepTimeouts timeouts;
  ...
  getSequenceStepEnables_x(&enables);
  getSequenceStepTimeouts_x(&enables, &timeouts);
...
/*bool setVcselPulsePeriod(vcselPeriodType type, uint8_t period_pclks, statInfo_t_VL53L0X *sensorInfo)
{
  ...
  setMeasurementTimingBudget(g_measTimBudUs, sensorInfo->address);
...
/*uint8_t getVcselPulsePeriod(vcselPeriodType type, statInfo_t_VL53L0X *sensorInfo)
{
  if (type == VcselPeriodPreRange)
  {
...
uint8_t getVcselPulsePeriod_x(vcselPeriodType type)
{
  if (type == VcselPeriodPreRange)
  {
...
uint16_t readRangeContinuousMillimeters(statInfo_t_VL53L0X *sensorInfo ) {
  ...
  if( sensorInfo == 0 ){
    ...
    temp = readReg16Bit_x(RESULT_RANGE_STATUS + 10,sensorInfo->address);
  } else {
    ...
    readMulti_x(0x14, tempBuffer, 12,sensorInfo->address);
    sensorInfo->rangeStatus =  tempBuffer[0x00]>>3;
    sensorInfo->spadCnt     = (tempBuffer[0x02]<<8) | tempBuffer[0x03];
    sensorInfo->signalCnt   = (tempBuffer[0x06]<<8) | tempBuffer[0x07];
    sensorInfo->ambientCnt  = (tempBuffer[0x08]<<8) | tempBuffer[0x09];
    temp                    = (tempBuffer[0x0A]<<8) | tempBuffer[0x0B];
    sensorInfo->rawDistance = temp;
  }
...
// extraStats provides additional info for this measurment. Set to 0 if not needed.
uint16_t readRangeSingleMillimeters(statInfo_t_VL53L0X *sensorInfo ) {
  writeReg_x(0x80, 0x01, sensorInfo->address);
  writeReg_x(0xFF, 0x01, sensorInfo->address);
  writeReg_x(0x00, 0x00, sensorInfo->address);
  writeReg_x(0x91, g_stopVariable, sensorInfo->address);
  writeReg_x(0x00, 0x01, sensorInfo->address);
  writeReg_x(0xFF, 0x00, sensorInfo->address);
  writeReg_x(0x80, 0x00, sensorInfo->address);
  writeReg_x(SYSRANGE_START, 0x01, sensorInfo->address);
  // "Wait until start bit has been cleared"
  startTimeout();
  while (readReg_x(SYSRANGE_START, sensorInfo->address) & 0x01){
    if (checkTimeoutExpired()){
      g_isTimeout = true;
      return 65535;
    }
  }
  return readRangeContinuousMillimeters( sensorInfo );
}
...
bool getSpadInfo(uint8_t * count, bool * type_is_aperture)
{
  ...
  {
	  HAL_UART_Transmit(&huart2, "hiiii", strlen("hiiii"), HAL_MAX_DELAY);
...
void getSequenceStepTimeouts(SequenceStepEnables const * enables, SequenceStepTimeouts * timeouts, statInfo_t_VL53L0X *sensorInfo)
{
  timeouts->pre_range_vcsel_period_pclks = getVcselPulsePeriod(VcselPeriodPreRange, sensorInfo->address);
  ...
  timeouts->final_range_vcsel_period_pclks = getVcselPulsePeriod(VcselPeriodFinalRange, sensorInfo->address);
...
void getSequenceStepTimeouts_x(SequenceStepEnables const * enables, SequenceStepTimeouts * timeouts)
{
  timeouts->pre_range_vcsel_period_pclks = getVcselPulsePeriod_x(VcselPeriodPreRange);
  ...
  timeouts->final_range_vcsel_period_pclks = getVcselPulsePeriod_x(VcselPeriodFinalRange);
