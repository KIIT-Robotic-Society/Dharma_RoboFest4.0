...
typedef struct{
  ...
  uint8_t  rangeStatus; //Ranging status (0-15)zz
  uint8_t address;
} statInfo_t_VL53L0X;
...
uint8_t setSignalRateLimit(float limit_Mcps, statInfo_t_VL53L0X *sensorInfo);

uint8_t setSignalRateLimit_x(float limit_Mcps);
...
uint8_t setMeasurementTimingBudget(uint32_t budget_us, statInfo_t_VL53L0X *sensorInfo);
uint8_t setMeasurementTimingBudget_x(uint32_t budget_us);
...
uint8_t setVcselPulsePeriod(vcselPeriodType type, uint8_t period_pclks,statInfo_t_VL53L0X *sensorInfo);
...
void writeReg_x(uint8_t reg, uint8_t value, statInfo_t_VL53L0X *sensorInfo);// Write an 8-bit register
...
uint8_t readReg_x(uint8_t reg, statInfo_t_VL53L0X *sensorInfo);// Read an 8-bit register
