...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
/*/void print_ibus_data(uint16_t *ibus_data, uint16_t length) {
	 char buffer[50];
	    for (int i = 0; i < length; i++) {
	        // Print each raw 16-bit value as a hexadecimal number
	        snprintf(buffer, sizeof(buffer), "Raw Channel %d: 0x%04X\n", i, ibus_data[i]);
