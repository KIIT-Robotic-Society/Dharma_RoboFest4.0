...
void setTimeout_x(uint16_t timeout){
  g_ioTimeout = timeout;
}

uint16_t getTimeout_x(void){
  return g_ioTimeout;
}
...
// based on VL53L0X_get_measurement_timing_budget_micro_seconds()
// in us
uint32_t getMeasurementTimingBudget_x(statInfo_t_VL53L0X *sensorInfo)
{
  SequenceStepEnables enables;
  SequenceStepTimeouts timeouts;
...
uint16_t readRangeSingleMillimeters( statInfo_t_VL53L0X *extraStats ) {
  ...
  /*startTimeout();
  while (readReg_x(SYSRANGE_START,  extraStats) & 0x01){
    if (checkTimeoutExpired()){
      g_isTimeout = true;
      return 65535;
    }
...
void startContinuous_x(uint32_t period_ms,statInfo_t_VL53L0X *sensorInfo)
{
  ...
  if (period_ms != 0)
  {
    ...
    uint16_t osc_calibrate_val = readReg16Bit_x(OSC_CALIBRATE_VAL, sensorInfo->address);
    ...
    writeReg32Bit_x(SYSTEM_INTERMEASUREMENT_PERIOD, period_ms, sensorInfo->address);

    // VL53L0X_SetInterMeasurementPeriodMilliSeconds() end

    writeReg_x(SYSRANGE_START, 0x04, sensorInfo->address); // VL53L0X_REG_SYSRANGE_MODE_TIMED
  }
  else
  {
    // continuous back-to-back mode
    writeReg_x(SYSRANGE_START, 0x02, sensorInfo->address); // VL53L0X_REG_SYSRANGE_MODE_BACKTOBACK
  }
}
...
void stopContinuous_x(statInfo_t_VL53L0X *sensorInfo)
{
  writeReg(SYSRANGE_START, 0x01); // VL53L0X_REG_SYSRANGE_MODE_SINGLESHOT

  writeReg_x(0xFF, 0x01, sensorInfo->address);
  writeReg_x(0x00, 0x00, sensorInfo->address);
  writeReg_x(0x91, 0x00, sensorInfo->address);
  writeReg_x(0x00, 0x01, sensorInfo->address);
  writeReg_x(0xFF, 0x00, sensorInfo->address);
}
...
// single-shot range measurement)
// extraStats provides additional info for this measurment. Set to 0 if not needed.
uint16_t readRangeContinuousMillimeters_x(statInfo_t_VL53L0X *sensorInfo ) {
  uint8_t tempBuffer[12];
  uint16_t temp;
  startTimeout();
...
// based on VL53L0X_PerformSingleRangingMeasurement()
// extraStats provides additional info for this measurment. Set to 0 if not needed.
uint16_t readRangeSingleMillimeters_x(statInfo_t_VL53L0X *sensorInfo ) {
  ...
  }  return readRangeContinuousMillimeters_x( sensorInfo );
}
